package com.rabbitmq.mq.exchange;
...
public class TopicExcechanges {
    ...
    /**
     * 将队列topic.QUEUE_MESSAGE_TEST与exchange绑定，binding_key为topic.message,就是完全匹配
     *
    ...
    Binding bindingExchangeValidateCode(Queue sendMessageQueue, TopicExchange exchange) {
        return BindingBuilder.bind(sendMessageQueue).to(exchange).with(QueueConstant.QUEUE_MESSAGE_TEST);
    }
